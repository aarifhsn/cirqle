...
const PostHeader = ({ post, onUnsave }) => {
    ...
    const handleSave = async () => {
        try {
            const res = await api.post(
                `${import.meta.env.VITE_SERVER_BASE_URL}/posts/${post.id}/save`,
            );
            setIsSaved(res.data.is_saved);
            toast.success(res.data.message);

            // if unsaved and we're on saved page, remove it
            if (!res.data.is_saved && onUnsave) {
                onUnsave(post.id);
            }
        } catch (e) {
